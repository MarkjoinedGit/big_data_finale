...
if __name__ == "__main__":
    ...
    # Input path
    input_path = os.path.join(absolute_path, 'input/input_41.csv')
    ...
    # Collect top K sensors
    top_k_sensors = sensors_counter.take(k)

    # Print the result on the standard output
    print(top_k_sensors)

    # Output path
    output_path = os.path.join(absolute_path, 'output','exercise-41')

    # Save data on the output file
    sc.parallelize(top_k_sensors).coalesce(1).saveAsTextFile(output_path)
