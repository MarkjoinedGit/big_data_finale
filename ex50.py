...
def perform_analysis_sql(session, df, output_path):
    ...
    df_2.repartition(1).write.csv(output_path, mode='overwrite', header=True)  
...
if __name__ == "__main__":
    ...
    # Input path
    input_path = os.path.join(absolute_path, 'input/input_50.csv')

    # Output paths
    output_df_path = os.path.join(absolute_path, 'output/exercise-50/result_dataframe_mode')
    output_sql_path = os.path.join(absolute_path, 'output/exercise-50/result_sql_mode')
